 ...
 * <b>shared</b> by all users/programs executing in the same context (i.e., on
 * the same CloudSpace server, or in the same current working directory on an
 * end-user machine). Its contents are <b>persistent</b>, meaning that they will
 * continue to be available from one program run to the next. Its contents are
 * <b>shared</b> meaning that everyone using this class on the same CloudSpace
 * server will see the same set of keys and values--objects stored in this map
 * will be visible to everyone, as long as the same identifying key is used.
 * Basically, it is a shared container where you can store information
 * semi-permanently, so that your program(s) or anyone else's can access it
 * again later.
 * <p>
 ...
 */
public class SharedPersistentMap<T> extends AbstractPersistentMap<T>
{
     ...
     * have been previously stored in other instances of this class, by
     * <em>any</em> program running in the same context (i.e., on the same
     * CloudSpace server, or in the same development project on a local
     * machine).
     * <p>
     * Typical usage: if you want a shared persistent map that stores objects of
     * class <code>Widget</code>, then:
     * </p>
     * 
     * <pre>
     * SharedPersistentMap&lt;Widget&gt; map = new SharedPersistentMap&lt;Widget&gt;( Widget.class );
     * </pre>
     * 
     * @param genericClass
     ...
     * <p>
     * Because the contents of this map might be shared among many users, <b>you
     * may not use this method in a program running in a multi-user context</b>
     * (i.e., it is forbidden on a CloudSpace server).
     * </p>
     */
    @Override
    public void clear()
    {
        ...
        {
            throw new UnsupportedOperationException( "clear() is not supported on a SharedPersistentMap whose "
