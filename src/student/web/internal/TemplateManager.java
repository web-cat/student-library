...
public class TemplateManager
{
	...
	private File templateStorageLocation; /*= new File(CloudSpaceConfiguration.getInstance().getStorageLocation(),"template/");*/
	...
	public class Template
	{
		...
		{
			name = persistedTemplate.getName().substring(0, persistedTemplate.getName().indexOf(".template"));
			...
			{
				//logger.error("Could not find the file specified in the template load "+persistedTemplate.getName(), e);
			}
			catch (IOException e)
			{
		...
		{
			Properties prop = new Properties();
			prop.setProperty(defaultField.getName() + "." + NULLABLE, new Boolean(defaultField.isNullable()).toString());
			for(Field f : fields)
			{
				prop.setProperty(f.getName() +"."+ NULLABLE, new Boolean(f.isNullable()).toString());
			}
			try
			{
		...
		public void removePersistence()
		{
			...
			{
				//logger.error("Could not delete Persisted Template " + this.name);
			}
